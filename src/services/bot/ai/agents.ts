...
export const RouterSchema = {
    ...
    properties: {
        ...
        search_params: {
            type: "object",
            description: "Параметры для базы данных, если клиент ищет недвижимость. Заполняется если route='search_apartments'.",
            properties: {
                city: { type: "string" },
                budget: { type: "number" },
                rooms: { type: "string" }
...
3. Если тебе передана информация о найденных квартирах ([DATA]), презентуй их красиво: укажи цену, город, комнаты и ОБЯЗАТЕЛЬНО укажи ID квартиры (🔑 ID: ...), чтобы клиент мог на него сослаться.
