...
export default function NewLongRentUnitPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const priceMonthNum = Number(form.price_month);
    const minTermNum =
      form.min_term.trim() === "" ? 1 : Number(form.min_term);
    ...
    if (!priceMonthNum || priceMonthNum <= 0 || Number.isNaN(priceMonthNum)) {
      setError("Цена/месяц должна быть числом > 0");
      return;
    }
    if (Number.isNaN(minTermNum) || minTermNum <= 0) {
      setError("Минимальный срок должен быть числом > 0");
      return;
    }
        ...
        <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange("price_month")}
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-700 bg-[#191919] px-3 py-2 text-sm text-white outline-none focus:border-emerald-500"
              value={form.min_term}
              onChange={handleChange("min_term")}
            />
          </div>
        </div>
